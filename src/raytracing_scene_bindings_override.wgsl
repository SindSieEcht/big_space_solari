...
// ─────────────────────────────────────────────────────────────────────────────
// big_space_solari: scale-aware ray-bias override of `bevy_solari::scene_bindings`.
//
// This is a VERBATIM copy of bevy_solari 0.18.1's
//   src/scene/raytracing_scene_bindings.wgsl
// with exactly three edits, each tagged `BIG_SPACE_SOLARI:`
//   1. RAY_T_MAX raised from the room-scale 100 km default.
//   2. A new RAY_T_MIN_SCALE constant.
//   3. trace_ray() raises its effective t_min with the ray origin's distance
//      from the render origin.
//
// The two constant lines are rewritten at load time from the `SolariRayBias`
// resource (whole-line string replacement), so the literal values below are
// only defaults. Everything else MUST stay identical to the upstream shader:
// the @group/@binding declarations and struct layouts are mirrored in Rust by
// bevy_solari's binder.rs, so any divergence breaks pipeline validation. Re-sync
// this file when bumping bevy_solari.
//
// Why scale the floor: at large render coordinates one f32 ULP (≈ |origin|·2⁻²³)
// dwarfs the fixed 1 mm RAY_T_MIN, so the fixed floor is swallowed and rays
// self-intersect the surface they start on (acne / shadow boiling). A floor
// proportional to length(ray_origin) tracks the ULP and stays above it. Near the
// render origin length·SCALE ≪ ray_t_min, so behaviour is byte-identical to
// stock for the walkable near field — this is a pure superset.
// ─────────────────────────────────────────────────────────────────────────────
...
// BIG_SPACE_SOLARI: RAY_T_MAX raised from upstream 100000.0f (100 km) so rays
// can reach real geometry at true scale. Rewritten at load time from SolariRayBias.t_max.
const RAY_T_MAX = 1000000000.0f;
// BIG_SPACE_SOLARI: self-intersection floor scales with distance from the render
// origin (see trace_ray). Rewritten at load time from SolariRayBias.scale.
const RAY_T_MIN_SCALE = 0.000001f;

const RAY_NO_CULL = 0xFFu;

fn trace_ray(ray_origin: vec3<f32>, ray_direction: vec3<f32>, ray_t_min: f32, ray_t_max: f32, ray_flag: u32) -> RayIntersection {
    // BIG_SPACE_SOLARI: raise the effective near plane with the ray origin's
    // distance from the render origin so it never drops below the local f32 ULP.
    // Clamp to ray_t_max so the interval is at worst empty (finds no hit) rather
    // than inverted. max(ray_t_min, …) keeps stock behaviour where length is small.
    let scaled_t_min = max(ray_t_min, length(ray_origin) * RAY_T_MIN_SCALE);
    let effective_t_min = min(scaled_t_min, ray_t_max);
    let ray = RayDesc(ray_flag, RAY_NO_CULL, effective_t_min, ray_t_max, ray_origin, ray_direction);
    var rq: ray_query;
